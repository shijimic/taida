...
| ディスクリプタ | 必須 | 任意 | 意味 |
|----------------|------|------|------|
| `BuildUnit` | `name: Str`, `target: Str`, `entry: Symbol` | `output: Str`, `assets: @[RouteAsset]`, `before: @[BuildHook]` | コンパイル成果物のディスクリプタ |
...
`BuildUnit.entry` は **関数シンボル** を指します。ファイルパスエントリは採用しません。ソースファイルやディレクトリは CLI のビルドルートパスで与え、ディスクリプタ内ではシンボル境界を使います。
...
- `files` はプロジェクトルート相対のグロブのみ受け付ける。絶対パス / `..` 区切り / `~` 展開は reject (`E1911`)
...
- `command`, `cwd`, `env`, `exit_code`, 標準出力 / 標準エラーの要約、開始 / 終了時刻、入力ディスクリプタハッシュ、出力ツリーハッシュを hook ログとビルド指紋に含めます
- hook が非ゼロ終了 / シグナル / タイムアウトで失敗した場合、ビルド全体が fail し、既存の正常ビルド出力は保持されます (本書 8 のトランザクショナル更新)
...
ビルドドライバは `.taida/build/.tmp-<unix-millis>-<rand8hex>/` をステージング領域として使います。
...
- すべての `BuildUnit` / `AssetBundle` / `BuildHook` がステージングへ書き込み、すべて成功したときだけ `<target>/<unit-name>/` 単位で `rename`(2) (POSIX) または `MoveFileExW(MOVEFILE_REPLACE_EXISTING)` (Windows) でアトミック置換
...
- 起動時の取り残しステージング掃除: ビルドドライバ起動時に `transaction.json` を読み、24 時間超過 / プロセス ID が現存しないステージングを削除し `.cleanup.log` に記録します (記録なしの削除は行いません)
