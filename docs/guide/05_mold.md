...
**`:` を欠いた具体型** (`Mold[Int]`) は、現状のパーサでは `Int` という名前の **型変数** として解釈されてしまいます。`Mold[Int] => MyMold[Int] = @()` と書いてしまうと、文法上は通っても意図とは異なる束縛が起こり、後続の使用箇所で型エラーが分かりにくく出ます。**具体型を使うときは必ず `:` を付ける** と覚えてください。`:` を欠いた具体型を診断する仕様は将来の言語仕様で検討中です。
