...
> **非末尾位置での相互再帰の扱い**: 非末尾位置で互いを呼び合う相互再帰は、Native バックエンド、および Native と同系統のコード生成を行う WASM プロファイルでは `[E0700]` で拒否されます。Interpreter と JS バックエンドでは実行できますが、深い呼び出しでスタックオーバーフローを起こします。バックエンド間の挙動差分とその回避策 (直接再帰への書き換え、アキュムレータパターン) は [末尾再帰最適化リファレンス](../reference/tail_recursion.md) を参照してください。
